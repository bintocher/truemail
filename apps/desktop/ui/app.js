...
document.getElementById('composeBtn').onclick=()=>{resetComposer();document.getElementById('compTitle').textContent=L('Новое письмо','New message');showView('composeView');};
...
const tbActions=[
  ...
  {k:'archive',t:'В архив',en:'Archive',on:true},{k:'trash',t:'Удалить',en:'Delete',on:true},{k:'spam',t:'В спам',en:'Spam',on:false},
...
function showPasswordConnection(config){accountPasswordProvider=config.provider;document.getElementById('accountUsername').value=config.username||document.getElementById('accountEmail').value.trim();document.getElementById('accountImapHost').value=config.imap?.host||'';document.getElementById('accountImapPort').value=config.imap?.port||993;document.getElementById('accountImapSecurity').value=config.imap?.security||'ssl';document.getElementById('accountSmtpHost').value=config.smtp?.host||'';document.getElementById('accountSmtpPort').value=config.smtp?.port||465;document.getElementById('accountSmtpSecurity').value=config.smtp?.security||'ssl';document.getElementById('accountPasswordRow').classList.remove('hidden');document.getElementById('accountPassword').focus();}
...
function openComposerForMessage(action){if(!activeMessage)return;resetComposer();
