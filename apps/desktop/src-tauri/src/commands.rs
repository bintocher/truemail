...
///
/// Известное ограничение: `Update::download` из плагина накапливает ответ
/// целиком в памяти и только потом сверяет подпись, оборвать загрузку по
/// размеру из нашего кода нельзя. Подменивший манифест обновлений может так
/// исчерпать память - но подменить его способен лишь тот, кто уже владеет
/// каналом обновлений, а подписанный пакет он всё равно не подделает.
