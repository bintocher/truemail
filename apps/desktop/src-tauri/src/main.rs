...
fn run() -> anyhow::Result<()> {
    ...
    let state = AppState {
        ...
        send_wakeup: Arc::new(tokio::sync::Notify::new()),
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
                ...
                .unwrap_or_else(commands::default_keybindings);
            {
                ...
                // (issue #41, specs/message-charset-decoding.md, S-009).
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::mail_rule_delete_confirmation,
            commands::reorder_mail_rules,
            commands::run_mail_rules,
            commands::continue_mail_rule_run,
            commands::last_mail_rule_run,
            commands::pending_mail_rule_runs,
            commands::list_sender_policies,
            commands::preview_sender_policy,
            commands::save_sender_policy,
            commands::delete_sender_policy,
            commands::start_sender_policy_sweep,
            commands::continue_sender_policy_sweep,
            commands::cancel_sender_policy_sweep,
            commands::pending_sender_policy_jobs,
            commands::list_ignored_conversations,
            commands::preview_ignore_conversation,
            commands::enable_ignore_conversation,
            commands::disable_ignore_conversation,
            commands::continue_ignore_job,
            commands::pending_ignore_jobs,
            commands::list_sender_sweep_rules,
            commands::preview_sender_sweep,
            commands::start_sender_sweep,
            commands::set_sender_sweep_enabled,
            commands::update_sender_sweep_mode,
            commands::delete_sender_sweep_rule,
            commands::continue_sender_sweep_job,
            commands::cancel_sender_sweep_job,
            commands::pending_sender_sweep_jobs,
            commands::failed_message_operations,
            commands::retry_message_operation,
            commands::discard_message_operation,
            commands::accounts_without_trash,
            ...
            commands::set_messages_pinned,
            commands::list_pinned_messages,
            commands::save_message_task,
            commands::get_message_task,
            commands::list_message_tasks,
            commands::complete_message_task,
            commands::reopen_message_task,
            commands::delete_message_task,
            commands::overdue_message_task_count,
            commands::due_task_reminders,
            commands::mark_task_reminders_shown,
            commands::snooze_task_reminder,
            commands::list_quick_steps,
            commands::save_quick_step,
            commands::delete_quick_step,
            commands::reorder_quick_steps,
            commands::bind_quick_step_slot,
            commands::apply_quick_step,
...
mod command_contract_tests {
    ...
    fn critical_user_flows_are_exposed_by_tauri_and_the_ui_bridge() {
        for command in [
            ...
            "check_for_update",
            "install_update",
            // S-082: команды правил перечислены явно - список правил, редактор,
            // изменение порядка и ручной прогон должны быть доступны мосту.
            "list_mail_rules",
            "save_mail_rule",
            "set_mail_rule_enabled",
            "delete_mail_rule",
            "reorder_mail_rules",
            "run_mail_rules",
            ...
            // blocked-senders.md S-050: блокировка отправителя, управление
            // списками и уборка уже полученных писем перечислены явно - без
            // любой из этих команд раздел отправителей перестаёт работать.
            "list_sender_policies",
            "preview_sender_policy",
            "save_sender_policy",
            "delete_sender_policy",
            "start_sender_policy_sweep",
            "continue_sender_policy_sweep",
            "cancel_sender_policy_sweep",
            "pending_sender_policy_jobs",
            // ignore-conversation.md S-051: включение игнорирования, список
            // переписок и прекращение с возвратом писем.
            "list_ignored_conversations",
            "preview_ignore_conversation",
            "enable_ignore_conversation",
            "disable_ignore_conversation",
            "continue_ignore_job",
            "pending_ignore_jobs",
            // sweep-by-sender.md S-047: запуск автоочистки, управление её
            // записями и отмена уборки.
            "list_sender_sweep_rules",
            "preview_sender_sweep",
            "start_sender_sweep",
            "set_sender_sweep_enabled",
            "update_sender_sweep_mode",
            "delete_sender_sweep_rule",
            "continue_sender_sweep_job",
            "cancel_sender_sweep_job",
            "pending_sender_sweep_jobs",
