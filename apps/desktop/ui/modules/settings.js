  ...
  ACCOUNT_COLORS.forEach(color=>{
    ...
    swatch.onclick=async()=>{
      try{
        ...
        account.color=color;
        card.querySelector('.account-ava').style.background=color;card.querySelector('.account-ava').style.color=contrastOn(color);
...
// Подробности состояния синхронизации почты в карточке аккаунта
// (mail-sync-visible-state.md, S-009): время последнего успеха, либо
// локализованный текст ошибки по её виду с исходным текстом в подробностях,
// либо, при needs_reauth, сообщение о повторном входе с действием
// переподключения вместо обычного текста ошибки. Текст и действие берутся из
// той же таблицы error-kinds-and-messages.md, что и во всплывающих сообщениях
// (errorPresentation), а не придумываются заново здесь.
function renderAccountSyncState(card,account){
  const el=card.querySelector('.account-sync-state');
  if(!el)return;
  el.innerHTML='';el.classList.remove('reauth');
  const state=window.mailSyncIndicator.resolveSyncIndicator(account,window.mailSyncTransient?.[account.id]);
  if(state.kind==='needs_reauth'||state.kind==='error'){
    const presented=window.errorPresentation.presentError(
      {kind:state.errorKind,message:state.message},
      {locale:wizardLocale,translations:typeof wizardText==='undefined'?undefined:wizardText,connected:true},
    );
    const text=document.createElement('span');text.className='account-sync-text';text.textContent=presented.text;el.appendChild(text);
    if(state.kind==='needs_reauth'){
      el.classList.add('reauth');
      // S-010: то же действие, что и кнопка "Переподключить" в этой же карточке.
      const action=document.createElement('button');action.type='button';action.className='btn sm account-sync-reauth';action.textContent=presented.actionLabel;action.onclick=()=>showAccountWizard(account.email);el.appendChild(action);
    }else{
      const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent=wt('errorDetails');const pre=document.createElement('pre');pre.textContent=state.message||'';details.append(summary,pre);el.appendChild(details);
    }
    return;
  }
  if(state.kind==='ok'){
    const date=window.mailSyncIndicator.parseUtcSqlDate(state.lastSyncAt);
    const text=document.createElement('span');text.className='account-sync-text';
    const formatted=date?date.toLocaleString(document.documentElement.lang):state.lastSyncAt;
    text.textContent=L(`Последняя синхронизация: ${formatted}`,`Last synced: ${formatted}`);
    el.appendChild(text);
  }
}

async function renderSignatureSettings(card,account){const body=card.querySelector('.cb'),panel=document.createElement('section');panel.className='signature-settings';panel.innerHTML=`<div class="t">${L('Подписи','Signatures')}</div><div class="d">${L('Разные подписи для новых писем и ответов.','Separate signatures for new messages and replies.')}</div><div class="signature-grid"></div>`;body.appendChild(panel);const grid=panel.querySelector('.signature-grid');try{const values=await accountSignatures(account.id,true);for(const kind of ['new','reply']){const value=values.find(item=>item.kind===kind)||{body_html:'',enabled:false};const item=document.createElement('div');item.className='signature-item';item.innerHTML=`<div class="signature-item-head"><strong>${kind==='new'?L('Новое письмо','New message'):L('Ответ','Reply')}</strong><label><input type="checkbox"${value.enabled?' checked':''}> ${L('Включена','Enabled')}</label></div><div class="signature-editor" contenteditable="true" data-ph="${L('Текст подписи','Signature text')}"></div><button class="btn sm signature-save" type="button">${L('Сохранить','Save')}</button>`;const editor=item.querySelector('.signature-editor'),enabled=item.querySelector('input');editor.innerHTML=value.body_html||'';item.querySelector('.signature-save').onclick=async()=>{try{await window.tm.saveSignature(account.id,kind,editor.innerHTML,enabled.checked);signatureCache.delete(account.id);await accountSignatures(account.id);showToast(L('Подпись сохранена','Signature saved'));}catch(error){showToast(error);}};grid.appendChild(item);}}catch(error){panel.querySelector('.d').textContent=window.errorPresentation.errorText(error,{locale:wizardLocale,translations:wizardText});}}

// Аккордеон карточек аккаунтов (accounts-accordion-password.md). Состояние -
// вне DOM (S-004): переживает фоновую перерисовку renderAccountSettings, а не
// теряется вместе со старыми карточками. openAccountRestored отделяет "ещё не
// читали localStorage" (undefined) от "уже восстановили и там пусто" (null) -
// иначе каждая перерисовка заново читала бы устаревшее значение хранилища
// поверх того, что пользователь только что переключил в этом сеансе.
const ACCOUNT_ACCORDION_KEY='truemail-account-open';
let openAccountId=null,openAccountRestored=false;
function readOpenAccountStorage(){try{return localStorage.getItem(ACCOUNT_ACCORDION_KEY);}catch(_){return null;}}
function writeOpenAccountStorage(id){try{if(id===null)localStorage.removeItem(ACCOUNT_ACCORDION_KEY);else localStorage.setItem(ACCOUNT_ACCORDION_KEY,String(id));}catch(_){/* S-005: недоступное хранилище не ломает раздел, аккордеон работает в пределах сеанса */}}
function applyOpenAccountDom(){document.querySelectorAll('#set-accounts .account-card').forEach(card=>{const isOpen=Number(card.dataset.accountId)===openAccountId;card.classList.toggle('open',isOpen);const toggle=card.querySelector('.account-toggle');if(toggle){toggle.setAttribute('aria-expanded',String(isOpen));toggle.setAttribute('aria-label',isOpen?L('Свернуть подробности','Collapse details'):L('Показать подробности','Show details'));}});}
function setOpenAccountId(id){openAccountId=id;writeOpenAccountStorage(id);applyOpenAccountDom();}

// Последние данные карточек аккаунтов (ui-language-switch.md, S-007). Смена
// языка перерисовывает карточки по ним, не обращаясь заново к ядру.
let lastAccountSettingsData=null;
window.relocalizeAccountSettings=function(){
  if(!lastAccountSettingsData)return;
  const {accounts,foldersByAccount,calendars}=lastAccountSettingsData;
  renderAccountSettings(accounts,foldersByAccount,calendars);
};
function renderAccountSettings(accounts,foldersByAccount,calendars){
  lastAccountSettingsData={accounts,foldersByAccount,calendars};
  ...
  accounts.forEach((account,index)=>{const folders=foldersByAccount[index]||[],accountCalendars=calendars.filter(cal=>cal.account_id===account.id);const card=document.createElement('div');card.className='card account-card';card.dataset.noI18n='1';card.dataset.accountId=String(account.id);const canChangePassword=accountCards.canChangeAccountPassword(account.auth_kind);card.innerHTML=`<div class="ch"><button type="button" class="iconbtn account-toggle" aria-expanded="false" aria-controls="acc-cb-${account.id}"><i data-i="down"></i></button><span class="ava ava-26 account-ava" style="background:${accountColorById(account.id)};color:${contrastOn(accountColorById(account.id))}"></span><div class="grow"><div class="account-name"></div><div class="account-email"></div><div class="account-stats"></div><div class="account-sync-state"></div></div><div class="account-actions"><button type="button" class="btn sm account-rename"><i data-i="edit"></i>${L('Переименовать','Rename')}</button><button type="button" class="btn sm account-folders"><i data-i="folder"></i>${L('Папки','Folders')}</button><button type="button" class="btn sm account-reconnect"><i data-i="lock"></i>${L('Переподключить','Reconnect')}</button>${canChangePassword?`<button type="button" class="btn sm account-password"><i data-i="key"></i>${L('Сменить пароль','Change password')}</button>`:''}</div></div><div class="cb" id="acc-cb-${account.id}"><div class="frow"><div class="fl"><div class="t">${L('Цвет аккаунта','Account color')}</div><div class="d">${L('Для аватаров писем и панели папок.','For message avatars and the folder panel.')}</div></div><div class="fc"><div class="account-colors"></div></div></div><div class="t" style="margin-top:14px">${L('Хранить письма локально','Keep mail locally')}</div><div class="d">${L('Письма старше выбранного срока автоматически удаляются из локального кэша (на сервере остаются). Свежие письма кэшируются целиком и открываются мгновенно.','Messages older than the selected period are automatically removed from the local cache (they stay on the server). Recent messages are cached in full and open instantly.')}</div><div class="account-retention"><input type="number" class="inp ret-num" min="1" max="999" value="1"><select class="sel ret-unit"><option value="1">${L('дней','days')}</option><option value="7">${L('недель','weeks')}</option><option value="30">${L('месяцев','months')}</option></select><label class="ret-unlim"><input type="checkbox" class="ret-unlimited"> ${L('без ограничений','no limit')}</label></div><div class="frow account-tls-row" style="margin-top:14px"><div class="fl"><div class="t">${L('Не проверять сертификат сервера','Do not check the server certificate')}</div><div class="d">${L('Нужно только для сервера с самоподписанным сертификатом. Пока проверка выключена, подмену сервера между программой и почтой ничто не выдаст: пароль и письма уйдут тому, кто подменил.','Needed only for a server with a self-signed certificate. While the check is off, nothing reveals a server substituted between the app and your mail: the password and messages go to whoever substituted it.')}</div></div><div class="fc"><div class="toggle account-tls-toggle"></div></div></div><div class="t" style="margin-top:14px">${L('Календари и адресные книги определяются автоматически по адресу аккаунта.','Calendars and address books are detected automatically from the account address.')}</div><div class="account-calendars"></div></div>`;card.querySelector('.ava').textContent=(account.display_name||account.email)[0].toUpperCase();card.querySelector('.account-toggle').setAttribute('aria-label',L('Показать подробности','Show details'));card.querySelector('.account-name').textContent=account.display_name||account.email;const emailCell=card.querySelector('.account-email');emailCell.textContent=account.email;emailCell.title=account.email;renderAccountSyncState(card,account);card.querySelector('.account-rename').onclick=async()=>{const name=prompt(L('Название аккаунта','Account name'),account.display_name||account.email);if(!name||name.trim()===account.display_name)return;try{await window.tm.renameAccount(account.id,name.trim());await window.reloadCoreData();showToast(L('Название аккаунта сохранено','Account name saved'));}catch(error){showToast(error);}};card.querySelector('.account-folders').onclick=()=>setSection('folders');card.querySelector('.account-reconnect').onclick=()=>showAccountWizard(account.email);card.querySelector('.account-password')?.addEventListener('click',()=>openChangePasswordDialog(account));
  // Состояние занимает отдельную строку карточки: длинная причина ошибки не
  // сжимает имя аккаунта и кнопки действий в шапке.
  const syncState=card.querySelector('.account-sync-state');
  card.querySelector('.ch').after(syncState);
  card.querySelector('.account-stats').textContent=accountCards.accountStatsText(
    folders.length,
  ...
  card.querySelector('.ch').addEventListener('click',event=>{if(event.target.closest('.account-actions'))return;setOpenAccountId(accountCards.nextOpenAccountId(openAccountId,account.id));});
  const chips=card.querySelector('.account-calendars');(accountCalendars.length?accountCalendars:[{name:L('Календарь ещё синхронизируется','Calendar is still syncing')}]).forEach(cal=>{const chip=document.createElement('span');chip.className='calendar-chip';chip.textContent=cal.name;chips.appendChild(chip);});
  ...
  if(tlsToggle){
    ...
    tlsToggle.addEventListener('click',async()=>{
      const next=!tlsToggle.classList.contains('on');
      tlsToggle.classList.toggle('on',next);
      try{
        await window.tm.setAccountTlsInsecure(account.id,next);
        await window.reloadCoreData();
        showToast(next
          ?L('Сертификат этого ящика больше не проверяется','The certificate of this mailbox is no longer checked')
          :L('Проверка сертификата включена','Certificate checking is on'));
      }catch(error){
        tlsToggle.classList.toggle('on',!next);
        showToast(error);
      }
  ...
  const applyRetention=async()=>{retNum.disabled=retUnlim.checked;retUnit.disabled=retUnlim.checked;const value=retUnlim.checked?0:Math.max(1,Number(retNum.value)||1)*Number(retUnit.value);try{await window.tm.setAccountRetention(account.id,value);account.retention_days=value;showToast(value?L(`Кэш: хранить ${value} дн.`,`Cache: keep ${value} days`):L('Кэш без ограничений','Cache without limit'));}catch(error){showToast(error);}};
  ...
  // Восстановление состояния аккордеона (S-001, S-003, S-004): читаем
  // localStorage один раз за сеанс, дальше держим значение в памяти.
  if(!openAccountRestored){openAccountId=accountCards.restoreOpenAccountId(readOpenAccountStorage(),accounts.map(a=>a.id));if(openAccountId===null)writeOpenAccountStorage(null);openAccountRestored=true;}
  else if(openAccountId!==null&&!accounts.some(a=>a.id===openAccountId)){openAccountId=null;writeOpenAccountStorage(null);}
  applyOpenAccountDom();
  const mapping=document.getElementById('set-folders');mapping.querySelectorAll('.mapping-generated').forEach(el=>el.remove());accounts.forEach((account,index)=>{const card=document.createElement('div');card.className='card mapping-generated';card.dataset.noI18n='1';card.innerHTML=`<div class="ch">${escapeHtml(account.email)}</div><div class="cb"></div>`;const body=card.querySelector('.cb'),folders=sortedFolders(foldersByAccount[index]||[]),roleTitles=smartIsEnglish()?{inbox:'Inbox',sent:'Sent',drafts:'Drafts',archive:'Archive',spam:'Spam',trash:'Trash'}:{inbox:'Входящие',sent:'Отправленные',drafts:'Черновики',archive:'Архив',spam:'Спам',trash:'Корзина'};['inbox','sent','drafts','archive','spam','trash'].forEach(role=>{const row=document.createElement('div');row.className='map-row map-2';row.innerHTML=`<div class="role">${roleTitles[role]}</div><select class="sel"><option value="">${L('Не назначено','Not assigned')}</option>${folders.map(folder=>`<option value="${folder.id}"${folder.role===role?' selected':''}>${escapeHtml(folder.display_name)}</option>`).join('')}</select>`;row.querySelector('select').onchange=async()=>{const value=row.querySelector('select').value;try{await window.tm?.setFolderRole(account.id,role,value?+value:null);await window.reloadCoreData?.();showToast(L('Сопоставление папки сохранено','Folder mapping saved'));}catch(error){showToast(error);}};body.appendChild(row);});mapping.appendChild(card);});
  const unified=document.getElementById('set-unified');unified.querySelectorAll('.unified-generated').forEach(el=>el.remove());const info=document.createElement('div');info.className='card unified-generated';info.dataset.noI18n='1';info.innerHTML=`<div class="ch"><span class="grow">${L('Источники писем для умных папок','Message sources for smart folders')}</span><span class="source-count"></span></div><div class="cb"><div class="unified-toolbar"><button type="button" class="btn sm" data-source-mode="all">${L('Выбрать все','Select all')}</button><button type="button" class="btn sm" data-source-mode="standard">${L('Только стандартные','Standard only')}</button></div><div class="unified-accounts"></div></div>`;const roleNames=smartIsEnglish()?{inbox:'Inbox',sent:'Sent',drafts:'Drafts',archive:'Archive',spam:'Spam',trash:'Trash'}:{inbox:'Входящие',sent:'Отправленные',drafts:'Черновики',archive:'Архив',spam:'Спам',trash:'Корзина'};window.coreUnifiedSettings=window.coreUnifiedSettings||{};// reloadList=true только когда пользователь сам поменял состав источников:
// filterSmart с resetScroll=true сбрасывает позицию списка и счётчик автодобора,
// а настройки перерисовываются на каждой фоновой перезагрузке данных - оттуда
// перезапускать список писем нельзя.
const refreshSourceCount=(reloadList=false)=>{const enabled=coreFolders.filter(folder=>window.coreUnifiedSettings[folder.id]!=='0').length;info.querySelector('.source-count').textContent=L(`${enabled} из ${coreFolders.length} включено`,`${enabled} of ${coreFolders.length} enabled`);if(reloadList&&currentSmartIndex!==null)filterSmart(currentSmartIndex);
  // Состав источников - такой же вход счёта, как условия самой умной папки:
  // ядро отбрасывает письма исключённых папок. Пересчёт зовём здесь, а не в
  // setSource: кнопки "выбрать все" и "только стандартные" переключают источники
  // пачкой, и на каждую папку проход по базе не нужен.
  if(reloadList)window.refreshSmartCounts?.(true);};const setSource=async(folder,enabled)=>{await window.tm.setUnifiedSource(folder.id,enabled);window.coreUnifiedSettings[folder.id]=enabled?'1':'0';const checkbox=info.querySelector(`[data-source-folder="${folder.id}"]`);if(checkbox)checkbox.checked=enabled;};const groups=info.querySelector('.unified-accounts');
  // Карта id->папка строится один раз на всю отрисовку списка источников, а не
  // на каждую строку - на неё опирается folderNames.folderPathLabel (S-002).
  const foldersByIdForLabels=new Map(coreFolders.map(folder=>[folder.id,folder]));
  // Список источников сортируется по показанной подписи, поверх порядка ролей;
  // общая sortedFolders (mail.js) не меняется - её использует ещё панель папок.
  const sourceSortedFolders=list=>list.map(folder=>({folder,label:folderNames.folderPathLabel(folder,foldersByIdForLabels)})).sort(folderNames.compareFolderLabels).map(item=>item.folder);
  accounts.forEach((account,index)=>{const section=document.createElement('section');section.className='unified-account';const title=document.createElement('div');title.className='unified-account-title';title.textContent=account.email;section.appendChild(title);sourceSortedFolders(foldersByAccount[index]||[]).forEach(folder=>{const row=document.createElement('label');row.className='unified-source';const enabled=window.coreUnifiedSettings[folder.id]!=='0';row.innerHTML=`<input type="checkbox" data-source-folder="${folder.id}"${enabled?' checked':''}><span class="source-path"></span><span class="source-role"></span>`;row.querySelector('.source-path').textContent=folderNames.folderPathLabel(folder,foldersByIdForLabels);row.querySelector('.source-role').textContent=roleNames[folder.role]||L('Без роли','No role');row.querySelector('input').onchange=async event=>{try{await setSource(folder,event.target.checked);refreshSourceCount(true);}catch(error){event.target.checked=!event.target.checked;showToast(error);}};section.appendChild(row);});groups.appendChild(section);});info.querySelector('[data-source-mode="all"]').onclick=async()=>{try{await Promise.all(coreFolders.map(folder=>setSource(folder,true)));refreshSourceCount(true);}catch(error){showToast(error);}};info.querySelector('[data-source-mode="standard"]').onclick=async()=>{try{await Promise.all(coreFolders.map(folder=>setSource(folder,Boolean(folder.role))));refreshSourceCount(true);}catch(error){showToast(error);}};refreshSourceCount();unified.appendChild(info);
...
// Диалог тихой смены пароля (S-009 - S-018, accounts-accordion-password.md).
// Разметка - #pwOverlay в index.html; сама операция идёт в ядре одним вызовом
// changeAccountPassword(accountId, password), без мастера и без переподключения.
function pwDialogLock(locked){
  document.getElementById('pwInput').disabled=locked;
  document.getElementById('pwSubmit').disabled=locked;
  document.getElementById('pwCancel').disabled=locked;
  document.getElementById('pwClose').disabled=locked;
  // Пока идёт проверка на сервере, диалог не закрывается (S-009): общий
  // обработчик Escape кликает по кнопке отмены, а она в этот момент отключена,
  // поэтому клик ничего не делает. Признак data-lock оставлен для наглядности
  // состояния в разметке.
  document.getElementById('pwOverlay').dataset.lock=locked?'1':'';
}
function closeChangePasswordDialog(){
  const overlay=document.getElementById('pwOverlay');
  if(overlay.dataset.lock==='1')return;
  overlay.classList.remove('open');
  document.getElementById('pwInput').value='';
  const status=document.getElementById('pwStatus');status.textContent='';status.dataset.kind='';
}
function openChangePasswordDialog(account){
  const overlay=document.getElementById('pwOverlay');
  document.getElementById('pwTitle').textContent=L('Сменить пароль','Change password');
  document.getElementById('pwAccount').textContent=account.display_name||account.email;
  document.getElementById('pwFieldLabel').textContent=L('Новый пароль','New password');
  document.getElementById('pwSubmitLabel').textContent=L('Проверить и сохранить','Check and save');
  document.getElementById('pwCancel').textContent=L('Отмена','Cancel');
  document.getElementById('pwClose').setAttribute('aria-label',L('Закрыть','Close'));
  const status=document.getElementById('pwStatus');status.textContent='';status.dataset.kind='';
  document.getElementById('pwInput').value='';
  overlay.dataset.accountId=String(account.id);
  pwDialogLock(false);
  overlay.classList.add('open');
  document.getElementById('pwInput').focus();
}
document.getElementById('pwClose').onclick=closeChangePasswordDialog;
document.getElementById('pwCancel').onclick=closeChangePasswordDialog;
document.getElementById('pwOverlay').addEventListener('click',event=>{if(event.target.id==='pwOverlay')closeChangePasswordDialog();});
document.getElementById('pwSubmit').onclick=async()=>{
  const overlay=document.getElementById('pwOverlay'),accountId=Number(overlay.dataset.accountId),password=document.getElementById('pwInput').value,status=document.getElementById('pwStatus');
  if(!password){status.textContent=L('Введите новый пароль','Enter the new password');status.dataset.kind='error';return;}
  pwDialogLock(true);status.dataset.kind='';status.textContent=L('Проверяю…','Checking…');
  try{
    await window.tm.changeAccountPassword(accountId,password);
    document.getElementById('pwInput').value='';
    overlay.classList.remove('open');
    showToast(L('Пароль сохранён','Password saved'));
  }catch(error){
    document.getElementById('pwInput').value='';
    status.dataset.kind='error';
    // Неверные учётные данные - единственный код с отдельным текстом (S-012);
    // остальные коды показываются как есть, интерфейс различает их не по тексту.
    // missing_secret_ref означает, что у аккаунта нет сохранённой ссылки на
    // пароль: менять нечего, помогает только переподключение.
    status.textContent=error?.code==='invalid_credentials'?L('Пароль не подошёл, прежний пароль сохранён','The password did not work, the previous password is kept')
      :error?.code==='missing_secret_ref'?L('У аккаунта нет сохранённого пароля, нужно переподключить ящик','The account has no stored password, reconnect the mailbox')
...
if(autostartToggle){
  autostartToggle.addEventListener('click',async()=>{
    ...
    catch(error){autostartToggle.classList.toggle('on');showToast(error);}
  });
}
...
if(sendToToggle){
  sendToToggle.addEventListener('click',async()=>{
    ...
    catch(error){sendToToggle.classList.toggle('on');showToast(error);}
  });
}
...
if(checkUpdatesButton)checkUpdatesButton.onclick=async()=>{const status=document.getElementById('updateStatus');checkUpdatesButton.disabled=true;if(status)status.textContent=L('Проверяю новую версию…','Checking for a new version…');try{const info=await window.tm.checkForUpdate();if(info.available_version){if(status)status.textContent=L(`Доступна версия ${info.available_version}`,`Version ${info.available_version} is available`);window.showUpdateButton?.(info.available_version,info.downloaded,info.notes);showToast(L(`Доступен truemail ${info.available_version}`,`truemail ${info.available_version} is available`),L('Обновить','Update'),async()=>{if(status)status.textContent=L('Скачиваю и устанавливаю обновление…','Downloading and installing the update…');if(window.startUpdateInstall)await window.startUpdateInstall();else await window.tm.installUpdate();});}else if(status)status.textContent=L(`Установлена актуальная версия ${info.current_version}`,`Version ${info.current_version} is up to date`);}catch(error){if(status)status.textContent=window.errorPresentation.errorText(error,{locale:wizardLocale,translations:wizardText});}finally{checkUpdatesButton.disabled=false;}};
...
window.applyCoreSettings=function(settings){
  ...
  try{smartCounterModes=JSON.parse(settings.smart_counters||'{}')||{};}catch(_){smartCounterModes={};}
  window.refreshSmartCounts?.();
