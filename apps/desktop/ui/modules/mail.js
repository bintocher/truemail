...
function filterTag(tag){clearMessageSelection();goMail();document.querySelectorAll('.navitem').forEach(item=>item.classList.remove('active'));currentTagName=tag.name;currentFolderId=null;currentSmartIndex=null;applyListOptions(true,tag.name);renderTagsNav();}
...
function filterSmart(index,resetScroll=true){currentSmartIndex=index;currentFolderId=null;currentTagName=null;applyListOptions(resetScroll,smartFolderTitle(smartFolders[index])||messagesTitle());loadSmartCoveragePage(index,true);}
