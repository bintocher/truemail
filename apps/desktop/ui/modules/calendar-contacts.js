...
// Единая точка закрытия вспомогательных меню (specs/single-popup-menu.md).
// Реестр знает, где искать узел каждого меню и как его закрыть; решение о том,
// что закрыть, принимает чистый модуль popup-menus.js. Узлы ищутся лениво: часть
// меню создаётся модулями, которые подключаются позже этого файла.
const POPUP_MENU_REGISTRY={
  message:{find:()=>[ctxmenu],isOpen:n=>n.classList.contains('open'),close:n=>n.classList.remove('open')},
  smart:{find:()=>[ctxsmart],isOpen:n=>n.classList.contains('open'),close:n=>n.classList.remove('open')},
  folder:{find:()=>[ctxfolder],isOpen:n=>n.classList.contains('open'),close:n=>n.classList.remove('open')},
  tag:{find:()=>[ctxtag],isOpen:n=>n.classList.contains('open'),close:n=>n.classList.remove('open')},
  contact:{find:()=>[ctxcontact],isOpen:n=>n.classList.contains('open'),close:n=>n.classList.remove('open')},
  attachment:{find:()=>[...document.querySelectorAll('.att-menu:not(.flag-menu)')],isOpen:()=>true,close:n=>n.remove()},
  flag:{find:()=>[...document.querySelectorAll('.flag-menu')],isOpen:()=>true,close:n=>n.remove()},
  more:{find:()=>[document.getElementById('threadMoreMenu')],isOpen:n=>n.classList.contains('open'),close:n=>{n.classList.remove('open');document.getElementById('threadMoreButton')?.setAttribute('aria-expanded','false');}},
  filter:{find:()=>[document.getElementById('filterMenu')],isOpen:n=>!n.classList.contains('hidden'),close:n=>n.classList.add('hidden')},
  sort:{find:()=>[document.getElementById('sortMenu')],isOpen:n=>!n.classList.contains('hidden'),close:n=>n.classList.add('hidden')},
  icon:{find:()=>[document.getElementById('smartIcons')],isOpen:n=>!n.classList.contains('hidden'),close:n=>n.classList.add('hidden')},
  color:{find:()=>[...document.querySelectorAll('.color-grid')],isOpen:n=>!n.classList.contains('hidden'),close:n=>n.classList.add('hidden')},
};
...
// key - элемент, к которому привязано меню; hover - открытие подменю наведением.
...
// Какое меню содержит узел: клик внутри открытого меню закрывают его пункты, а
// не общий обработчик (S-014, S-015).
function popupMenuAtNode(node){return openPopupMenuIds().find(id=>popupMenuNodes(id).some(menu=>menu.contains(node)))||null;}
// Escape: пока открыто хотя бы одно меню, модальные окна не трогаем (S-011, S-012).
// Событие помечаем, чтобы второй обработчик Escape не закрыл окно следом.
function handlePopupMenusEscape(event){
  if(event?.popupMenusClosed)return true;
  if(!openPopupMenuIds().length)return false;
  applyPopupMenuAction({type:'escape'});
  popupMenuAnchor={id:null,key:null};
  if(event)event.popupMenusClosed=true;
  return true;
}
...
// Номер открытия подменю меток: список меток приходит из ядра асинхронно, и к
// этому моменту родительское меню может закрыться, а подменю - открыться заново
// от другого пункта. Узел добавляем, только если наше открытие последнее (S-010).
let flagMenuTicket=0;
async function openFlagMenu(message,event,hover=false){
  ...
  let labels=[],active=[];
  try{[labels,active]=await Promise.all([window.tm.listLabels(),window.tm.messageLabelIds(message.id)]);}catch(error){showToast(error);return;}
  ...
  labels.forEach(label=>{
    ...
    item.onclick=async e=>{e.stopPropagation();const on=!activeSet.has(label.id);try{await window.tm.toggleMessageLabel(message.id,label.id,on);if(on)activeSet.add(label.id);else activeSet.delete(label.id);item.querySelector('.flag-check').textContent=on?'✓':'';await window.reloadCoreData?.();}catch(error){showToast(error);}};
...
function buildContextMenu(){
  ...
  (smartIsEnglish()?[['flag','flag','Add tag'],['important-toggle','check','Set or clear flag'],['pin-message','pin','Pin or unpin'],['task-edit','cal','Remind about message'],['raw','edit','View source'],['eml','download','Save as .eml'],['create-rule','filter','Create rule'],['block-sender','shield','Block sender'],['ignore-conversation','snooze','Ignore conversation'],['sweep-sender','trash','Clean up by sender'],['delete-forever','trash','Delete permanently']]:[['flag','flag','Добавить метку'],['important-toggle','check','Поставить или снять флажок'],['pin-message','pin','Закрепить или открепить'],['task-edit','cal','Напомнить о письме'],['raw','edit','Исходный текст'],['eml','download','Сохранить как .eml'],['create-rule','filter','Создать правило'],['block-sender','shield','Заблокировать отправителя'],['ignore-conversation','snooze','Игнорировать переписку'],['sweep-sender','trash','Автоочистка по отправителю'],['delete-forever','trash','Удалить навсегда']]).forEach(([act,icon,label])=>{const item=document.createElement('div');item.className='tmi'+(act==='flag'?' has-submenu':'');item.dataset.contextAction=act;item.innerHTML=`<i data-i="${icon}"></i>${label}`;if(act==='flag')item.addEventListener('mouseenter',()=>{if(activeMessage){const rect=item.getBoundingClientRect();openFlagMenu(activeMessage,{clientX:rect.right+2,clientY:rect.top},true);}});ctxmenu.appendChild(item);});
  renderIcons(ctxmenu);
}
// Пункт меню письма закрывает и само меню, и открытое подменю меток (S-008).
ctxmenu.addEventListener('click',async event=>{const item=event.target.closest('[data-context-action]');if(!item)return;closePopupMenus(['message']);const action=item.dataset.contextAction;
  ...
  if(action==='important-toggle'&&activeMessage){const ids=selectedMessageIds.size?[...selectedMessageIds]:[activeMessage.id];window.setMessagesFlaggedFromUi?.(ids,!activeMessage.flags?.flagged,activeMessage);return;}
  if(action==='pin-message'&&activeMessage){const ids=selectedMessageIds.size?[...selectedMessageIds]:[activeMessage.id];window.setMessagesPinnedFromUi?.(ids,!activeMessage.pinned_at);return;}
  if(action==='task-edit'&&activeMessage){window.openMessageTaskEditor?.(activeMessage);return;}
  if(action.startsWith('quick-step:')){window.executeQuickStep?.(Number(action.slice(11)));return;}
  if(action==='flag'){openFlagMenu(activeMessage,event);return;}
  executeToolbarAction(action);
});
ctxfolder.querySelectorAll('[data-folder-action]').forEach(item=>item.addEventListener('click',async()=>{if(item.classList.contains('disabled')||!contextFolder)return;const action=item.dataset.folderAction;if(action==='open'){contextFolderOpen?.();return;}if(action==='settings'){showView('settingsView');setSection('folders');return;}if(action==='count-total'||action==='count-unread'){const key=action==='count-total'?'t':'u';const set=new Set((folderCounterModes[contextFolder.id]||'u').split('').filter(c=>c==='u'||c==='t'));set.has(key)?set.delete(key):set.add(key);folderCounterModes[contextFolder.id]=['u','t'].filter(k=>set.has(k)).join('')||'n';window.tm.setSetting('folder_counters',JSON.stringify(folderCounterModes)).catch(console.error);item.classList.toggle('on',set.has(key));updateFolderBadge(document.querySelector(`.folder-row[data-folder-id="${contextFolder.id}"]`),contextFolder);return;}if(action==='create'){const name=prompt(L(`Имя новой папки внутри «${contextFolder.display_name}»`,`New folder name inside "${contextFolder.display_name}"`),'');if(!name||!name.trim())return;try{await window.tm.createFolder(contextFolder.account_id,contextFolder.id,name.trim());await window.reloadCoreData();showToast(L('Папка создана на сервере','Folder created on the server'));}catch(error){showToast(error);}return;}
  if(action==='rename'){const name=prompt(L('Новое имя папки','New folder name'),contextFolder.display_name);if(!name||name.trim()===contextFolder.display_name)return;try{await window.tm.renameFolder(contextFolder.id,name.trim());await window.reloadCoreData();showToast(L('Папка переименована на сервере','Folder renamed on the server'));}catch(error){showToast(error);}return;}if(action==='delete'){if(!await confirmAction(L(`Удалить папку «${contextFolder.display_name}» на сервере? Письма внутри также будут удалены.`,`Delete the folder "${contextFolder.display_name}" on the server? Messages inside will also be deleted.`)))return;try{await window.tm.deleteFolder(contextFolder.id);await window.reloadCoreData();showToast(L('Папка удалена на сервере','Folder deleted on the server'));}catch(error){showToast(error);}}}));
ctxcontact.querySelectorAll('[data-contact-action]').forEach(item=>item.addEventListener('click',async()=>{if(item.classList.contains('disabled')||!contextContact)return;const action=item.dataset.contactAction,email=contextContact.emails?.[0]?.email;if(action==='edit'){openContactEditor(contextContact);return;}if(action==='compose'){resetComposer();setRecipients('compTo',[{name:contextContact.display_name||'',email}]);document.getElementById('compTitle').textContent=L('Новое письмо','New message');showView('composeView');await applyComposerSignature('new');return;}if(action==='copy'){try{await navigator.clipboard.writeText(email);showToast(L('Email скопирован','Email copied'));}catch(error){showToast(error);}return;}if(action==='delete'){if(!await confirmAction(L(`Удалить контакт «${contextContact.display_name||email||''}»?`,`Delete contact "${contextContact.display_name||email||''}"?`)))return;try{await window.tm.deleteContact(contextContact.id);await window.reloadCoreData();showToast(L('Контакт удалён','Contact deleted'));}catch(error){showToast(error);}}}));
