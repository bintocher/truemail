...
function openQuickStepEditor(source){
  ...
  overlay.querySelector('.quick-step-action-add').onclick=()=>{if(host.children.length<quickStepsModel.MAX_ACTIONS)host.appendChild(quickStepActionRow({kind:'mark_read'},host));else showToast(quickStepsModel.quickStepErrorText('actions_limit',ruleLang()));};
...
document.getElementById('quickStepAdd')?.addEventListener('click',()=>{if(quickStepSnapshot.length>=quickStepsModel.MAX_STEPS){showToast(L(`Предел быстрых действий: ${quickStepsModel.MAX_STEPS}`,`Quick step limit: ${quickStepsModel.MAX_STEPS}`));return;}openQuickStepEditor({sort_order:quickStepSnapshot.length});});
