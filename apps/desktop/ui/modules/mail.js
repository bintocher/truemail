...
async function loadSmartCoveragePage(index,reset=false){
  ...
  try{const existing=reset?[]:(coreSmartRows.get(folder.id)||[]),cursor=existing.at(-1);
    ...
    // из которых состоит умная папка, и повторяем запрос.
    if(!rows.length&&cursor?.date){const folderIds=[...new Set(existing.map(message=>message.folder_id))];let fetchedAny=false;for(const folderId of folderIds){const source=coreFolders.find(item=>item.id===folderId);const localCount=messages.filter(message=>message.folder_id===folderId).length;if(source&&(source.total_count||0)>localCount){try{if((await window.tm?.fetchOlderMessages(folderId,cursor.date,SMART_MESSAGE_PAGE_SIZE))>0)fetchedAny=true;}catch(error){console.error('truemail smart backfill:',error);}}}if(fetchedAny)rows=await window.tm.listSmartFolderMessages(folder.id,cursor?(cursor.date||''):null,cursor?.id||null,SMART_MESSAGE_PAGE_SIZE);}
    const known=new Set(existing.map(message=>message.id)),combined=[...existing,...rows.filter(message=>!known.has(message.id))];coreSmartRows.set(folder.id,combined);smartHasMore.set(folder.id,rows.length>0);const byId=new Map(messages.map(message=>[message.id,message]));rows.forEach(message=>byId.set(message.id,message));messages=[...byId.values()];
  }catch(error){console.error('smart folder coverage',error);}finally{loadingSmartCoverage=false;window.setListLoading?.(false);if(currentSmartIndex===index&&currentFolderId===null)applyListOptions(false);if(smartOverlay.classList.contains('open'))updateSmartPreview();const queued=queuedSmartCoverage;queuedSmartCoverage=null;if(queued)loadSmartCoveragePage(queued.index,queued.reset);}
}
