...
- Версия внизу боковой панели снова видна. Подпись заполнялась один раз при
  загрузке окна и молча оставалась пустой, если мост к ядру в этот момент был
  ещё не готов; в журнале при этом не появлялось ни строки. Теперь запрос
  повторяется до готовности моста, неудача попадает в журнал, а сама подпись
  прижата к нижнему краю панели и не уезжает за прокрутку длинного списка папок.
