...
// Вход, из которого показывать нечего. Все случаи собраны вместе: они об одном
// и том же - окно "что нового" не должно рисовать ни пустую строку, ни голую
// решётку вместо заголовка.
test('описание без содержания не даёт узлов', () => {
  const empty = [
    ['', 'пустая строка'],
    [null, 'описания нет вовсе'],
    [undefined, 'описание не передано'],
    ['   \n\n  \n', 'одни пробелы и переводы строк'],
    ['###\n', 'решётка без текста заголовка'],
  ];
  empty.forEach(([source, why]) => {
    assert.deepEqual(parseReleaseNotes(source), [], `${why}: в окне появится пустой узел`);
  });
});
