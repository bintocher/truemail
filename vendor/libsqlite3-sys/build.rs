...
mod build_bundled {
    ...
    pub fn main(out_dir: &str, out_path: &Path) {
        ...
        if cfg!(feature = "bundled-sqlcipher") {
            verify_bundled_sqlcipher_versions();
        }
    ...
    fn verify_bundled_sqlcipher_versions() {
        const EXPECTED_SQLITE: &str = "#define SQLITE_VERSION        \"3.53.3\"";
        const EXPECTED_SQLCIPHER: &str = "#define CIPHER_VERSION_NUMBER 4.17.0";

        let sqlite_header = std::fs::read_to_string("sqlcipher/sqlite3.h")
            .expect("failed to read bundled SQLCipher sqlite3.h");
        assert!(
            sqlite_header.contains(EXPECTED_SQLITE),
            "bundled SQLCipher must use SQLite 3.53.3"
        );

        let sqlcipher_source = std::fs::read_to_string("sqlcipher/sqlite3.c")
            .expect("failed to read bundled SQLCipher sqlite3.c");
        assert!(
            sqlcipher_source.contains(EXPECTED_SQLCIPHER),
            "bundled SQLCipher must be version 4.17.0"
        );
    }
