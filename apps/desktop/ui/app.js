...
async function loadNextMessagePage(){
  ...
  try{
    ...
    if(currentFolderId!==null)renderMessageList(messages.filter(message=>message.folder_id===currentFolderId).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||b.id-a.id),coreFolders.find(folder=>folder.id===currentFolderId)?.display_name);else if(currentSmartIndex!==null)filterSmart(currentSmartIndex);
...
function smartOperators(field){return field.type==='text'?['contains','not_contains','equals','not_equals','starts_with','ends_with']:field.type==='date'?['within_last','older_than','before','after','on']:['equals','not_equals'];}
function normalizeSmartCondition(condition={}){
  ...
  return {f:definition.id,o:operator,v:String(value),...(definition.type==='date'&&['within_last','older_than'].includes(operator)?{u:condition.u||'hours'}:{})};
}
...
function smartConditionDescription(source){const condition=normalizeSmartCondition(source),field=smartField(condition.f),operator=smartOps[condition.o]?.[smartIsEnglish()?1:0]||condition.o;let value=condition.v;
  if(field.values)value=smartOptionLabel(field.values.find(item=>item[0]===condition.v)||[condition.v,condition.v,condition.v]);else if(field.type==='date'&&['within_last','older_than'].includes(condition.o)){const unit=smartUnits.find(item=>item[0]===condition.u)||smartUnits[1];value=`${condition.v} ${smartUnitLabel(unit,condition.v)}`;}else if(field.type==='text')value=`«${value}»`;
