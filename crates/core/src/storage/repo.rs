...
impl Db {
        ...
        for (path, uids) in vanished {
            for uid in uids {
                ...
                if let Some((id, raw_ref, fixed_id, pinned_at)) = row {
                    ...
                    let task = sqlx::query_as::<_, MessageTaskRow>(MESSAGE_TASK_BY_ID_SQL)
                        .bind(id)
                        .fetch_optional(&mut *tx)
                        .await?
                        .map(Into::into);
                        ...
                        && (pinned_at.is_some() || task.is_some())
                    {
                        ...
                        .await?;
                        if matches.0 == 1 {
                            store_transferred_message_traits(
                                &mut tx, account_id, &fixed_id, pinned_at, task,
                            )
                            .await?;
                        }
                    }
    ...
    pub async fn list_pinned_messages(
        ...
        let mut ordinary = self
            .ordinary_page_for_view(&mut snapshot, view_kind, view_value)
            .await?;
        ...
        Ok(match view_kind {
            "folder" => {
                let folder_id = view_value
                    .and_then(|value| value.parse::<i64>().ok())
                    .unwrap_or_default();
                self.list_messages(folder_id, PAGE).await?
            }
            "label" => {
...
async fn store_transferred_message_traits(
    ...
    let value = serde_json::to_string(&TransferredMessageTraits { pinned_at, task })?;
    sqlx::query(
