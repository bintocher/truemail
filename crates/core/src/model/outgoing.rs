...
/// Проверить окно отмены письма, принимаемого в очередь, вместе с его
/// происхождением. Окна нет ни у отправки по времени, ни у служебного письма
/// программы, ни у письма локального интерфейса приложений: ноль у них
/// технический, а не выбор пользователя, и наименьшее окно отмены его не
/// касается (S-055, S-057, S-059). Иначе поднятое наименьшее окно отклоняло бы
/// автоответ и отправку по времени целиком.
pub fn validate_send_undo_seconds(
    origin: &str,
    value: i64,
    limits: &LimitSet,
) -> Result<i64, String> {
    if origin != SEND_ORIGIN_ORDINARY {
        return if value == 0 {
            Ok(0)
        } else {
            Err(format!(
                "отправка с происхождением {origin} идёт без окна отмены, получено {value}"
            ))
        };
    }
    validate_undo_seconds(value, limits)
}
